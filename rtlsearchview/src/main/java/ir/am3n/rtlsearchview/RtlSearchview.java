...
import ir.am3n.rtlsearchview.adapters.CursorSearchAdapter;
import ir.am3n.rtlsearchview.db.HistoryContract;
import ir.am3n.rtlsearchview.utils.AnimationUtils;
 ...
 */
public class RtlSearchview extends FrameLayout {

    //region Properties
    /**
     * The freaking log tag. Used for logs, duh.
     */
    private static final String LOG_TAG = RtlSearchview.class.getSimpleName();
    ...
    /**
     * Whether or not the RtlSearchview will animate into view or just appear.
     */
    private boolean mShouldAnimate;
    
    /**
     * Whether or not the RtlSearchview will clonse under a click on the Tint View (Blank Area).
     */
    ...
    private boolean mVoiceHide = false;
    ...
    private ListView mSelectionsListView;
    private boolean mSelectionsEnabled = false;
    ...
    //region Constructors
    public RtlSearchview(Context context) {
        this(context, null);
    }

    public RtlSearchview(Context context, AttributeSet attributeSet) {
        this(context, attributeSet, 0);
    }

    public RtlSearchview(Context context, AttributeSet attributeSet, int defStyleAttributes) {
    ...
    private void init() {
        ...
        mRoot.bringToFront();
        mStatusbar = mRoot.findViewById(R.id.view_statusbar);
    ...
    private void initStyle(AttributeSet attributeSet, int defStyleAttribute) {
        ...
        TypedArray typedArray = mContext.obtainStyledAttributes(attributeSet, R.styleable.RtlSearchview, defStyleAttribute, 0);

        if (typedArray != null) {

            if (typedArray.hasValue(R.styleable.RtlSearchview_searchStatusbarBackgroundColor)) {
                setStatusbarBackgroundColor(typedArray.getColor(R.styleable.RtlSearchview_searchStatusbarBackgroundColor, Color.GRAY));
            }
            if (typedArray.getBoolean(R.styleable.RtlSearchview_android_fitsSystemWindows, false)) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    mStatusbar.getLayoutParams().height = getStatusBarHeight();
                }
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_searchBackground)) {
                setBackground(typedArray.getDrawable(R.styleable.RtlSearchview_searchBackground));
            }
            if (typedArray.hasValue(R.styleable.RtlSearchview_searchBackgroundColor)) {
                setBackgroundColor(typedArray.getColor(R.styleable.RtlSearchview_searchBackgroundColor, Color.WHITE));
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_android_textColor)) {
                setTextColor(typedArray.getColor(R.styleable.RtlSearchview_android_textColor,
                        ContextCompat.getColor(mContext,R.color.black)));
            }
            if (typedArray.hasValue(R.styleable.RtlSearchview_searchEdtTextSize)) {
                setSearchEdtTextSize(typedArray.getDimensionPixelSize(R.styleable.RtlSearchview_searchEdtTextSize, 0));
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_android_textColorHint)) {
                setHintTextColor(typedArray.getColor(R.styleable.RtlSearchview_android_textColorHint,
                        ContextCompat.getColor(mContext,R.color.gray_50)));
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_android_hint)) {
                setHint(typedArray.getString(R.styleable.RtlSearchview_android_hint));
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_searchVoiceIcon)) {
                setVoiceIcon(typedArray.getResourceId(
                        R.styleable.RtlSearchview_searchVoiceIcon,
                        R.drawable.ic_action_voice_search)
                );
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_searchVoiceHide)) {
                setVoiceHide(typedArray.getBoolean(
                        R.styleable.RtlSearchview_searchVoiceHide,
                        false)
                );
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_searchCloseIcon)) {
                setClearIcon(typedArray.getResourceId(
                        R.styleable.RtlSearchview_searchCloseIcon,
                        R.drawable.ic_action_navigation_close)
                );
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_searchBackIcon)) {
            ...
            if (typedArray.hasValue(R.styleable.RtlSearchview_searchSuggestionBackground)) {
                setSuggestionBackground(typedArray.getResourceId(
                        R.styleable.RtlSearchview_searchSuggestionBackground,
                        R.color.search_layover_bg)
                );
            }
            if (typedArray.hasValue(R.styleable.RtlSearchview_searchSuggestionBackgroundColor)) {
                setSuggestionBackgroundColor(typedArray.getColor(
                        R.styleable.RtlSearchview_searchSuggestionBackgroundColor,
                        getResources().getColor(R.color.search_layover_bg))
                );
            }
            if (typedArray.hasValue(R.styleable.RtlSearchview_searchSelectionBackgroundColor)) {
                setSelectionBackgroundColor(typedArray.getColor(
                        R.styleable.RtlSearchview_searchSelectionBackgroundColor,
                        getResources().getColor(R.color.search_layover_bg))
                );
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_historyIcon) && mAdapter instanceof CursorSearchAdapter) {
                ((CursorSearchAdapter)mAdapter).setHistoryIcon(typedArray.getResourceId(
                        R.styleable.RtlSearchview_historyIcon,
                        R.drawable.ic_history_white));
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_suggestionIcon) && mAdapter instanceof CursorSearchAdapter) {
                ((CursorSearchAdapter)mAdapter).setSuggestionIcon(typedArray.getResourceId(
                        R.styleable.RtlSearchview_suggestionIcon,
                        R.drawable.ic_action_search_white));
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_listTextColor) && mAdapter instanceof CursorSearchAdapter) {
                ((CursorSearchAdapter)mAdapter).setTextColor(typedArray.getColor(R.styleable.RtlSearchview_listTextColor,
                        ContextCompat.getColor(mContext,R.color.white)));
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_android_inputType)) {
                setInputType(typedArray.getInteger(
                        R.styleable.RtlSearchview_android_inputType,
                        InputType.TYPE_CLASS_TEXT)
                );
            }


            if (typedArray.hasValue(R.styleable.RtlSearchview_searchSuggestionEnabled)) {
                mSuggestionsEnabled = typedArray.getBoolean(R.styleable.RtlSearchview_searchSuggestionEnabled, false);
                mSuggestionsListView.setVisibility(mSuggestionsEnabled ? VISIBLE : GONE);
            }
            if (typedArray.hasValue(R.styleable.RtlSearchview_searchSelectionEnabled)) {
                mSelectionsEnabled = typedArray.getBoolean(R.styleable.RtlSearchview_searchSelectionEnabled, false);
                mSelectionsListView.setVisibility(mSelectionsEnabled ? VISIBLE : GONE);
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_searchBarHeight)) {
                setSearchBarHeight(typedArray.getDimensionPixelSize(R.styleable.RtlSearchview_searchBarHeight, getAppCompatActionBarHeight()));
            } else {
                setSearchBarHeight(getAppCompatActionBarHeight());
            }

            if (typedArray.hasValue(R.styleable.RtlSearchview_voiceHintPrompt)) {
                setVoiceHintPrompt(typedArray.getString(R.styleable.RtlSearchview_voiceHintPrompt));
            } else {
    ...
    private void initSearchView() {
        ...
        mSearchEditText.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                ...
                RtlSearchview.this.onTextChanged(s);
            }
    ...
    private void displayVoiceButton(boolean display) {
        ...
        if (!mVoiceHide && display && isVoiceAvailable()) {
    ...
    private void displayClearButton(boolean display) {
        mClear.setVisibility(display ? VISIBLE : View.GONE);
        if (!display)
            mPrbWait.setVisibility(GONE);
    }
     ...
     * @param start
     */
    public void openSearch(View start) {
        ...
        if (mShouldAnimate) {
            if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                ...
                startView = start;
                AnimationUtils.circleRevealView(startView, mSearchBar);
            }
            else {
    ...
    public void closeSearch() {
        ...
        if (mShouldAnimate) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                AnimationUtils.circleHideView(startView, mSearchBar, listenerAdapter);
            } else {
    ...
    private void onSubmitQuery() {
        ...
        if(query != null && TextUtils.getTrimmedLength(query) > 0) {
            ...
            if (mOnQueryTextListener == null || !mOnQueryTextListener.onQueryTextSubmit(query.toString())) {
                ...
                if (mSuggestionsEnabled && mShouldKeepHistory) {
    ...
    public void setTintAlpha(int alpha) {
        ...
        Drawable d = mSearchBar.getBackground();
    ...
    public void setSearchEdtTextSize(int size) {
    ...
    public void setVoiceHide(boolean flag) {
        mVoiceHide = flag;
        mVoice.setVisibility(mVoiceHide ? GONE : VISIBLE);
    }
    ...
    public void setSelectionBackgroundColor(int color) {
        mSelectionsListView.setBackgroundColor(color);
    }
    ...
    public void setSelectionsListViewAdapter(ListAdapter adapter) {
        mSelectionsListView.setAdapter(adapter);
    }
    public ListView getSelectionsListView() {
        return mSelectionsListView;
    }
...
//    }
    ...
    **/
    ...
    public void startProgressBarWait() {
        if (mClear.getVisibility()==VISIBLE) {
            mPrbWait.setAlpha(0f);
            mPrbWait.setVisibility(VISIBLE);
            mPrbWait.animate().alpha(1f).setDuration(300).start();
            mClear.animate().alpha(0f).setDuration(150).start();
        }
    }

    public void endProgressBarWait() {
        mPrbWait.animate().alpha(0f).setDuration(150).setListener(new Animator.AnimatorListener() {
            @Override public void onAnimationStart(Animator animation) {}
            @Override
            public void onAnimationEnd(Animator animation) {
                mPrbWait.setVisibility(GONE);
                animation.removeListener(this);
                mPrbWait.animate().setListener(null);
            }
            @Override
        ...
        mClear.animate().alpha(1f).setDuration(200).start();
    }
