package ir.am3n.rtlsearchview.utils;
...
public class AnimationUtils {
    ...
    @TargetApi(21)
    public static void circleRevealView(View start, View view, int duration) {
        // get the center for the clipping circle
        int cx = (start.getRight()+start.getLeft()) / 2;
        int cy = (start.getBottom()+start.getTop()) / 2;

        // get the final radius for the clipping circle
        float finalRadius = (float) Math.hypot(view.getMeasuredWidth(), view.getMeasuredHeight()/2);
    ...
    @TargetApi(21)
    public static void circleRevealView(View start, View view) {
        circleRevealView(start, view, ANIMATION_DURATION_SHORT);
    }

    @TargetApi(21)
    public static void circleHideView(View start, final View view, AnimatorListenerAdapter listenerAdapter) {
        circleHideView(start, view, ANIMATION_DURATION_SHORT, listenerAdapter);
    }

    @TargetApi(21)
    public static void circleHideView(View start, final View view, int duration, AnimatorListenerAdapter listenerAdapter) {
        ...
        // get the final radius for the clipping circle
        float initialRadius = (float) Math.hypot(view.getMeasuredWidth(), view.getMeasuredHeight()/2);
